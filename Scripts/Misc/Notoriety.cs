...
namespace Server.Misc
{
    public class NotorietyHandlers
	{
		...
		public static int MobileNotoriety( Mobile source, Mobile target )
		{
			...
			if ( target.Kills >= 5 || ( target.Body.IsMonster && IsSummoned( target as BaseCreature ) ) || ( target is BaseCreature && ( ( (BaseCreature)target ).AlwaysMurderer || ( (BaseCreature)target ).IsAnimatedDead ) ) )
				return Notoriety.Murderer;
